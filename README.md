...
- ✅ **详细信息**：获取视频标题、UP 主、播放数据、封面图等完整信息
...
6. **信息格式化**：将视频信息格式化为易读的文本
7. **消息回复**：发送格式化后的信息和封面图片
...
3. **封面图片**：视频封面图片通过 `Image` 消息段发送（有概率会发送失败，原因未知）
4. **简介长度**：视频简介超过 100 字符会自动截断并添加省略号
5. **无需权限**：插件对所有用户开放，无需管理员权限
